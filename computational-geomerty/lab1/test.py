...
def get_angle(p1: tuple, p2: tuple):
...
with open('vertices.txt') as points_file:
...
def draw(graph, point: tuple[int, int], draw_weight):
    ...
    for p in graph.out_edges:
        for edge in graph.out_edges[p]:
            ...
            plt.plot([p[0], edge.out_p[0]], [p[1], edge.out_p[1]], color='black')
            plt.arrow(p[0], p[1], dx / length, dy / length, color='red', width=0.1)
            if draw_weight:
